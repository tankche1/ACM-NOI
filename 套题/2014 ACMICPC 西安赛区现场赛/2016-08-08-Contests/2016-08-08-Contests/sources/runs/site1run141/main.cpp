...
int nnn, mmm, kkk, color[1000], BAOLI;
int check() {
    static int vis[1000], ccc = 0;
    ccc++;

    int cnt = 0;
    for(int i = 1; i <= nnn; i++) {
        if( vis[color[i]] != ccc ) vis[color[i]] = ccc, cnt++;
    }
    return cnt == kkk;
}
void dfs(int cur = 1) {
    ...
    for(int c = 1; c <= mmm; c++) if(c != color[cur - 1]){
        color[cur] = c;
...
int main() {
    ...
    for(int kase = 1; kase <= T; kase++) {
        ...
        if(k == 1) {
            ...
            ///static int CC[MAXK + 10];
            ///CC[0] = 1;
            ///for(int i = 1; i <= k; i++)
            ///    CC[i] = 1LL * CC[i - 1] * (n - i + 1) % MOD * inv[i] % MOD;

            int CC = 1;
            for(int i = 0; i < k; i++) {
                if(i != 0) CC = 1LL * CC * (k - i + 1) % MOD * inv[i] % MOD;
                int tmp = 1LL * CC * (k - i) % MOD * Pow(k - i - 1, n - 1) % MOD;
