...
const int INF=~0U>>1;
int first[maxn],nnext[maxm],u[maxm],v[maxm],edge=0,dis[maxm],fa[maxn];bool rem[maxn];
int d[maxn];
bool used[maxn];
int n,m;
...
int main(){
	...
	while(tcase--){
		...
		for(int i=0;i<(int)List.size();i++){
			rem[List[i]]=1;rem[List[i]^1]=1;Dijkstra();ans=max(ans,d[n]);
			rem[List[i]]=0;rem[List[i]^1]=0;
		}
