...
const int N = int(20) + 9; // 100 100 10000
Int fact[N*N], B[N][N][N*N];
int n;
...
int main(){
    ...
    n = 10; fact[0] = 1; REP_1(i, n*n) fact[i] = fact[i-1] * i;
    ...
    REP_1(i, n){
        ...
        REP_1(j, i) REP_1(k, i*j){
            B[i][j][k] = binom(i*j, k);
            REP_1(ii, i) FOR_1(jj, 0, j) FOR_1(kk, 0, min(k, ii*jj)) if (b(ii, jj, kk) && (i-ii)*(j-jj) >= k-kk){
                if (i == ii && j == jj) break;
                B[i][j][k] -= binom(i-1, ii-1) * binom(j, jj) * b(ii, jj, kk) * binom((i-ii)*(j-jj), k-kk);
            }
        }
    }
