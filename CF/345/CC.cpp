...
bool cmpx(int c,int d){
    return px[c]<px[d];
}

bool cmpy(int c,int d){
    return py[c]<py[d];
}
...
int main(){
    ...
    for(int i=1;i<=tot;i++){
        ...
        sort(bh+i,bh+j+1,cmpx);
        for(int k=i+1;k<=j;k++){
            if(px[bh[k]]==px[bh[k-1]]){
                Union(bh[k],bh[k-1]);
            }
        }

        sort(bh+i,bh+j+1,cmpy);
        for(int k=i+1;k<=j;k++){
            if(py[bh[k]]==py[bh[k-1]])
                Union(bh[k],bh[k-1]);
        }

        for(int k=i;k<=j;k++){
            tuan[find(bh[k])]=max(tuan[find(bh[k])],dp[bh[k]]);
        }

        for(int k=i;k<=j;k++){
            dp[bh[k]]=max(dp[bh[k]],tuan[find(bh[k])]);
        }
